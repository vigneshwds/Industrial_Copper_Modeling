...
def Selling_Price():
    ...
    with st.form('Selling_price'):
        ...
        with col1:
            quantity=st.number_input(label='Quantity Tons', min_value=0.1, max_value=6.9249)
            ...
            thickness=st.number_input(label='Thickness', min_value=0.1, max_value=3.2815)
        ...
        with col3:
            ...
            width=st.number_input(label='Width', min_value=700.0, max_value=1980.0)
            st.write('')
            st.write('')
    ...
    if Button:
        ...
        #Make prediction
        data1=np.array([[quantity, customer, country, status_int, item_int, application, thickness, width, prod_ref]])
...
def Predict_Status():
    ...
    with st.form('Status'):
        ...
        with col3:
            ...
            customer=st.number_input(label='Customer ID', min_value=30147616.0, max_value=2147483647.0)
            sell_price=st.number_input(label='Selling Price', min_value=0, max_value=2500)
            application=st.selectbox(label='Application code', options=application_options)
            width=st.number_input(label='Width', min_value=700.0, max_value=1980.0)
            st.write('')
            st.write('')
    ...
    if Button:
        ...
        #Make prediction
        data2=np.array([[quantity, customer, country, item_int, application, thickness, width, prod_ref, sell_price]])
...
with st.sidebar:
    user=option_menu('Menu', ['Home', 'Prediction', 'About'])

if user=='Home':
    home()
elif user=='Prediction':
    option=option_menu('', ['Selling Price Prediction', 'Transaction Status Prediction'], orientation='horizontal')
    ...
    #st.header(':violet[About this Project]')
